...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        identity = x
        x = self.conv(x)
        x = x + identity  # Non-inplace addition for gradient safety
        return self.relu(x)
            ...
            if i == self.index:
                ...
                for t, m in zip(parallel_res_list, self.down_conv_lists[-1]):
                    if x is None:
                        ...
                        x = x + m(t)  # Non-inplace addition for gradient safety
            else:
                ...
                if i != self.index - 1:
                    ...
                    for t, m in zip(res_list, self.up_conv_lists[i]):
                        if up_x is None:
                            ...
                            up_x = up_x + m(t)  # Non-inplace addition for gradient safety
                    x = x + up_x  # Non-inplace addition for gradient safety
                ...
                if i != 0:
                    ...
                    for t, m in zip(res_list, self.down_conv_lists[i - 1]):
                        if down_x is None:
                            ...
                            down_x = down_x + m(t)  # Non-inplace addition for gradient safety
                    x = x + down_x  # Non-inplace addition for gradient safety
            x = self.relu(x)
